...
function setTheme(dark) {
  ...
  themeToggle.title = dark ? 'حالت روز' : 'حالت شب';
...
changePassBtn.onclick = () => {
  ...
  if (!cur || !nw) { settingsMsg.textContent = 'هر دو فیلد رو پر کن.'; settingsMsg.style.color = 'var(--danger)'; return; }
...
regenInviteBtn.onclick = () => {
  if (confirm('یه کد دعوت جدید بسازم؟ کد قبلی دیگه کار نمی‌کنه.')) s.emit('adminSetInviteCode', { code: null });
};
...
s.on('adminUsersList', (d) => {
  ...
  d.users.forEach(u => {
    ...
    name.textContent = u.username + (u.isAdmin ? ' (مدیر)' : '') + (u.online ? ' • آنلاین' : '');
    row.append(av, name);
    if (u.username !== me && !u.isAdmin) {
      const rm = document.createElement('button');
      rm.type = 'button';
      rm.className = 'admin-remove-btn';
      rm.textContent = 'حذف از گروه';
      rm.onclick = () => { if (confirm(u.username + ' از گروه حذف بشه؟ دیگه نمی‌تونه وارد بشه.')) s.emit('adminRemoveUser', { username: u.username }); };
      row.appendChild(rm);
    }
...
s.on('kicked', () => {
  alert('حساب شما توسط مدیر گروه حذف شد.');
  location.reload();
});
...
function renderSidebar() {
  convList.innerHTML = '';
  convList.appendChild(buildConvItem('group', 'گروه تیم', null));
...
function buildConvItem(conversationId, title, dmUser) {
  ...
  else { av.style.background = 'linear-gradient(135deg,#37AEE2,#2894c7)'; av.textContent = 'گ'; }
  ...
  if (info.lastMessage) {
    const prefix = info.lastMessage.username === me ? 'تو: ' : '';
    previewEl.textContent = prefix + (info.lastMessage.text || (info.lastMessage.hasAttachment ? '📎 پیوست' : ''));
  } else {
    previewEl.textContent = 'هنوز پیامی نیست';
  }
...
function baseStatusText() {
  if (activeConv === 'group') {
    ...
    return { text: `${onlineCount} نفر آنلاین از ${otherUsers.length + 1}`, color: '#8b98a5' };
  }
  const other = otherFromDm(activeConv);
  if (onlineUsers.has(other)) return { text: 'آنلاین', color: '#4fbf6b' };
  const t = lastSeenMap[other];
  if (t) {
    const d = new Date(t), now = new Date();
    const sameDay = d.toDateString() === now.toDateString();
    const time = d.toLocaleTimeString('fa-IR', { hour: '2-digit', minute: '2-digit' });
    return { text: 'آخرین بازدید ' + (sameDay ? time : d.toLocaleDateString('fa-IR') + ' ' + time), color: '#8b98a5' };
  }
  return { text: 'آفلاین', color: '#8b98a5' };
}

function updateChatHeader() {
  if (activeConv === 'group') {
    chatTitle.textContent = 'گروه تیم';
    headAvatar.classList.remove('has-photo');
    headAvatar.style.backgroundImage = '';
    headAvatar.textContent = 'گ';
...
function updateTypingLine() {
  ...
  statusLine.textContent = names.length === 1 ? `${names[0]} داره می‌نویسه...` : 'چند نفر دارن می‌نویسن...';
...
// بعد از حذف/ویرایش، پیش‌نمایش سایدبار رو با آخرین پیامی که واقعاً توی چت مونده هماهنگ می‌کنه
...
function startReply(m) {
  cancelEdit();
  replyingTo = { id: m.id, username: m.username, text: m.text || (m.attachment ? '📎 پیوست' : '') };
  replyBarName.textContent = m.username === me ? 'پاسخ به خودت' : 'پاسخ به ' + m.username;
...
// ===== منوی راست‌کلیک / لمس‌ممتد (مثل تلگرام) =====
...
function showContextMenu(e, m) {
  ...
  ctxItem(actions, 'پاسخ', ICONS.reply, () => { startReply(m); closeContextMenu(); });
  if (m.text) ctxItem(actions, 'کپی متن', ICONS.copy, () => { copyText(m.text); closeContextMenu(); });
  ctxItem(actions, 'چه کسانی دیدن', ICONS.eye, () => showSeenList(menu, m));
  if (m.username === me) {
    if (m.text) ctxItem(actions, 'ویرایش', ICONS.edit, () => { startEdit(m); closeContextMenu(); });
    ctxItem(actions, 'حذف', ICONS.trash, () => { deleteMsg(m); closeContextMenu(); }, true);
  }
...
function showSeenList(menu, m) {
  ...
  header.textContent = 'چه کسانی دیدن';
  ...
  if (relevant.length === 0) {
    ...
    empty.textContent = 'کس دیگه‌ای توی این گفتگو نیست.';
  ...
  relevant.sort((a, b) => a.localeCompare(b)).forEach(u => {
    ...
    mark.textContent = seen ? '✓ دیده' : 'ندیده';
...
function add(m) {
  ...
  meta.className = 'meta';
  if (m.edited) { const tag = document.createElement('span'); tag.className = 'edited-tag'; tag.textContent = 'ویرایش‌شده'; meta.appendChild(tag); }
...
function renderAttachment(a) {
  if (a.type === 'image') {
    ...
    img.className = 'attach-image'; img.src = a.url; img.alt = a.name || 'عکس';
  ...
  link.textContent = '📄 ' + (a.name || 'دانلود فایل');
  return link;
}
...
function updateTicks(id) {
  ...
  if (seenBy.length > 0) {
    ...
    rec.ticksEl.title = 'دیده شد توسط ' + seenBy.join('، ');
    ...
    countEl.textContent = activeConv === 'group' && seenBy.length > 1 ? `دیده شد (${seenBy.length})` : 'دیده شد';
  } else {
    rec.ticksEl.classList.remove('seen');
    rec.ticksEl.innerHTML = singleTickSvg();
    rec.ticksEl.title = 'ارسال شد';
  }
}
