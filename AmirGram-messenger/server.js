...
// ===================== ذخیره‌سازی روی فایل (بدون نیاز به دیتابیس) =====================
...
// آخرین بازدید فقط در حافظه نگه داشته می‌شه (نه فایل)، چون کم‌اهمیت‌تره
const lastSeen = {}; // username -> ISO time

// ===================== رمزنگاری رمز عبور (بدون پکیج اضافه) =====================
...
io.on('connection', (socket) => {
  socket.on('register', ({ username, password, inviteCode }) => {
    ...
    if (!username || !password) return socket.emit('authError', 'نام کاربری و رمز را وارد کن.');
    if (String(inviteCode || '') !== settings.inviteCode) return socket.emit('authError', 'کد دعوت اشتباه است.');
    if (findUser(username)) return socket.emit('authError', 'این نام کاربری قبلاً گرفته شده.');
  ...
  socket.on('login', ({ username, password }) => {
    ...
    password = String(password || '');
    const u = findUser(username);
    if (!u || !verifyPassword(password, u.passwordHash)) return socket.emit('authError', 'نام کاربری یا رمز اشتباه است.');
  ...
  socket.on('sendMessage', ({ conversationId, text, attachment, replyTo }) => {
    ...
    const m = {
      ...
      replyTo: original ? { id: original.id, username: original.username, text: original.text || (original.attachment ? '📎 پیوست' : '') } : null,
  ...
  socket.on('changePassword', ({ oldPassword, newPassword }) => {
    ...
    if (!verifyPassword(String(oldPassword || ''), u.passwordHash)) {
      return socket.emit('accountMessage', { ok: false, text: 'رمز فعلی اشتباهه.' });
    }
    const np = String(newPassword || '');
    if (np.length < 4) return socket.emit('accountMessage', { ok: false, text: 'رمز جدید باید حداقل ۴ کاراکتر باشه.' });
    u.passwordHash = hashPassword(np);
    saveUsers();
    socket.emit('accountMessage', { ok: true, text: 'رمزت آپدیت شد.' });
  });

  // ===================== پنل مدیریت (فقط برای مدیر گروه) =====================
